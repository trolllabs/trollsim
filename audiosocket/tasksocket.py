...
def main():
	port = 5065
	track1 = 'assets/tasks/task1.wav'
	track2 = 'assets/tasks/task2.wav'
	track3 = 'assets/tasks/task3.wav'
	meta = {20: 'i'}
	...
	while True:
		...
		while True:
			...
			print('Incoming data: %s, length %s' % (data.hex().upper(), len(data)))
			packet_id, packet_value = parse_packet(meta, data)
			print(packet_id, type(packet_id), packet_value, type(packet_value))
			if packet_id == 20:
				if packet_value == 1:
