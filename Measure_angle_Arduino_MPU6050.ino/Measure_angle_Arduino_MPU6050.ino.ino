...
int angle_roll_buffer, angle_pitch_buffer, angle_yaw_buffer;
...
void loop(){
 ...
 //Subtract the offset values from the raw gyro values
  ...
  //angle_yaw_acc = asin((float)acc_z/acc_total_vector)* -57.296;       //Calculate the yaw angle
  
  angle_roll_acc -= 0.0;                                              //Accelerometer calibration value for roll
  angle_pitch_acc -= 0.0;                                               //Accelerometer calibration value for pitch
  //angle_yaw_acc -= 0.0;                                                //Accelerometer calibration value for yaw

  if(set_gyro_angles){                                                 //If the IMU is already started
    angle_roll = angle_roll * 0.9996 + angle_roll_acc * 0.0004;     //Correct the drift of the gyro roll angle with the accelerometer roll angle
    angle_pitch = angle_pitch * 0.9996 + angle_pitch_acc * 0.0004;        //Correct the drift of the gyro pitch angle with the accelerometer pitch angle
    //angle_yaw = angle_yaw * 0.9996 + angle_yaw_acc * 0.0004;           //Correct the drift of the gyro yaw angle with the accelerometer yaw angle
  }
  else{                                                                //At first start
    angle_roll = angle_roll_acc;                                     //Set the gyro roll angle equal to the accelerometer roll angle 
    angle_pitch = angle_pitch_acc;                                       //Set the gyro pitch angle equal to the accelerometer pitch angle
    //angle_yaw = angle_yaw_acc;                                         //Set the gyro yaw angle equal to the accelerometer yaw angle 
    set_gyro_angles = true;                                            //Set the IMU started flag
  }
  
  //To dampen the roll and pitch angles a complementary filter is used
  angle_roll_output = angle_roll_output * 0.9 + angle_roll * 0.1;   //Take 90% of the output roll value and add 10% of the raw roll value
  angle_pitch_output = angle_pitch_output * 0.9 + angle_pitch * 0.1*-1;      //Take 90% of the output pitch value and add 10% of the raw pitch value
  //angle_yaw_output = angle_yaw_output * 0.9 + angle_yaw * 0.1;      //Take 90% of the output pitch value and add 10% of the raw pitch value
  Serial.print(" | roll angle  = "); Serial.print(angle_roll_output);
  Serial.print(" | pitch angle  = "); Serial.println(angle_pitch_output);
  //Serial.print(" | Yaw angle  = "); Serial.println(angle_yaw_output);
